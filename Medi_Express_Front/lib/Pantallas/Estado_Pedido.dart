...
class EstadoPedidoScreen extends StatefulWidget {
  final String orderId;
  final String? status; // opcional: si no se pasa, se genera dinámicamente

  const EstadoPedidoScreen({super.key, required this.orderId, this.status});

  @override
  State<EstadoPedidoScreen> createState() => _EstadoPedidoScreenState();
}

class _EstadoPedidoScreenState extends State<EstadoPedidoScreen> {
  String computeDisplayStatus(AppLocalizations l10n) {
    // Si el widget fue invocado con orderId '0' (placeholder), usar latestOrderId
    final resolvedId = (widget.orderId == '0' || widget.orderId.isEmpty) ? OrderService.instance.latestOrderId.value ?? widget.orderId : widget.orderId;
    final order = OrderService.instance.getOrderById(resolvedId);
    String displayStatus = widget.status ?? l10n.noOrders;
    if (order != null && order.isNotEmpty) {
      final st = order['status'] ?? 'pendiente';
      final dispatchPoint = order['dispatchPoint'] ?? '';
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder<List<Map<String, String>>>(
      ...
      builder: (context, _, __) {
        final displayStatus = computeDisplayStatus(l10n);
        return Scaffold(
